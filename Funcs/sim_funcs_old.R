## base functions used in FM simulation and CFM simulation
# Fri Nov  2 10:50:38 2018 ------------------------------
...
# function for simulate datasets: dx X and dz Z
sim.data <- function(n, m, cor.pcs, sd.pcs, cov.xz, sd.e, dx, dz) {
  ...
  # covariance matrix between xi and z
  XZ <- matrix(0, ncol = dz, nrow = 3*dx)
  for (i in 1:(3*dx)) {
    vars <- floor((i-1)/3) + 1
    rep.row <- c(sapply(1:dz, function(x) {ifelse(x==vars, cov.xz, 0)}))
    XZ[i,] <- rep.row
  }
  # covariance between Z: assume they are independent, not relevant to our problem
  ZZ <- matrix(0, ncol = dz, nrow = dz)
  diag(ZZ) <- rep(1, 10)
  # final covariance matrix for generating random variables
  V <- matrix(0, ncol = 3*dx+dz, nrow = 3*dx+dz)
  V[1:(3*dx), 1:(3*dx)] <- XX
  V[1:(3*dx), (3*dx+1):(3*dx+dz)] <- XZ
  V[(3*dx+1):(3*dx+dz), 1:(3*dx)] <- t(XZ)
  V[(3*dx+1):(3*dx+dz), (3*dx+1):(3*dx+dz)] <- ZZ
  #rownames(V) <- append(c(sapply(seq(10, 100, by = 10), function(x) {x+c(1,2,3)})), paste("z", 1:10, sep = ""))
  #colnames(V) <- append(c(sapply(seq(10, 100, by = 10), function(x) {x+c(1,2,3)})), paste("z", 1:10, sep = ""))
  ...
  # observed W on S domain for all variables, measurement error is iid with mean zero and variance 0.25 for all varaibles
  ...
  return(list(Xt = X_t, Ws = W_s, Zt = Z_t, s = s_obs, t = t_obs, V = V, xi = xi_z[, 1:(3*dx)]))
}

# asychronous data simulation, 10 X and 10 Z: using presentation by Multivariate Karhunen–Loève Theorem
sim.data2 <- function(n, m, sd.pcs, cov.XZ, sd.e, basis.method = "split") {
  K <- length(sd.pcs)
  # generate t(response) observations
  t_obs <- t(sapply(1:n, function(x) {sort(runif(m, min = 0, max = 10))}))
  # generate s(covariate) observations
  ...
  V <- matrix(0, ncol = K+10, nrow= K+10)
  V[1:K, 1:K] <- diag(sd.pcs^2)
  V[1:K, (K+1):(K+10)] <- cov.XZ
  V[(K+1):(K+10), 1:K] <- t(cov.XZ)
  V[(K+1):(K+10), (K+1):(K+10)] <- diag(1, 10)
  xi_z <- mvrnorm(n, mu = rep(0, K+10), Sigma = V)
  # eigenfunctions construction
  # fourier basis functions
  if (basis.method == "split") {
    f.basis <- create.fourier.basis(rangeval = c(0, 100), nbasis = ifelse(K%%2 == 0, K+1, K))
  } else {
  ...
  X_t <- NULL
  for (v in 1:10) {
    meanvt <- mean_fun(t_obs, v = v)
    meanvs <- mean_fun(s_obs[[v]], v= v)
    # random sign: -1 or 1
    signv <- sample(c(-1, 1), size = 1, prob = c(0.5, 0.5))
    if (basis.method == "split") {
      X_s <- cbind(X_s, unlist(lapply(1:n, function(x) {
        meanvs[x,] + signv * eval.basis(s_obs[[v]][x,]+(v-1)*10, f.basis)[,1:K] %*% xi_z[x, 1:K]})))
      X_t <- cbind(X_t, unlist(lapply(1:n, function(x) {
        meanvt[x,] + signv * eval.basis(t_obs[x,]+(v-1)*10, f.basis)[,1:K] %*% xi_z[x, 1:K]})))
    } else {
      X_s <- cbind(X_s, unlist(lapply(1:n, function(x) {
        meanvs[x,] + signv * eval.basis(s_obs[[v]][x,], f.basis)[,1:K] %*% VD[[v]] %*% xi_z[x, 1:K]
      })))
      X_t <- cbind(X_t, unlist(lapply(1:n, function(x) {
        meanvt[x,] + signv * eval.basis(t_obs[x,], f.basis)[,1:K] %*% VD[[v]] %*% xi_z[x, 1:K]
      })))
    }
  }
  # observed W on s domain, adding measurement error
  W_s <- X_s + sapply(1:10, function(x) {rnorm(m*n, mean = 0, sd = sd.e)})
  # it's called Z_t, but time-invariant variables
  Z_t <- apply(xi_z[, (K+1):(K+10)], 2, function(x) {rep(x, each = 5)})
  return(list(Xt = X_t, Ws = W_s, Xs = X_s, Zt = Z_t, s = s_obs, t = t_obs, V = V))
}

# function for simulate datasets, only X variables
